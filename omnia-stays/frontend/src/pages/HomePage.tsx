  ...
  return (
    <PageTransition>
      ...
      <div id="cities" className="scroll-mt-0">
        {city ? (
          <div className="pt-16">
            <CityStays city={city} onBack={() => setCity(null)} />
          </div>
        ) : (
...
function CityStays({ city, onBack }: { city: string; onBack: () => void }) {
  const { data: listings = [], isLoading } = useQuery({
    queryKey: ['listings', city],
    queryFn: () => fetchListings({ city }),
  });
  ...
  return (
    <section className="px-6 pb-section">
      <Reveal className="mx-auto mb-6 flex max-w-6xl items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h2 className="font-serif text-2xl font-semibold tracking-tight sm:text-3xl">
            Stays in {city}
            <span className="ml-2 align-middle text-sm font-normal text-white/40">{listings.length} places</span>
          </h2>
        </div>
      </Reveal>
      ...
      {isLoading ? (
        ...
        <p className="py-20 text-center text-white/40">No stays found. Is the backend running on :3001?</p>
      ) : (
        <motion.div variants={stagger(0.05)} initial="hidden" animate="show" className="mx-auto grid max-w-6xl gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {listings.map((l, i) => (
            <ListingCard key={l.id} listing={l} featured={i === 0 && listings.length > 2} />
          ))}
        </motion.div>
      )}
    </section>
