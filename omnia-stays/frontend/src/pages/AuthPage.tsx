...
const appearance = {
  ...
  elements: {
    card: 'glass-strong shadow-[0_30px_80px_-30px_rgba(37,99,235,0.35)]',
    socialButtonsBlockButton: 'border border-slate-300 text-slate-800 hover:bg-slate-100',
    formButtonPrimary: 'bg-[linear-gradient(110deg,#1d4ed8,#2563eb_45%,#3b82f6)] text-[#fff] text-sm normal-case',
    ...
    <PageTransition>
      <div className="flex min-h-[100svh] flex-col items-center justify-center px-6 py-28">
        ...
        <p className="mb-6 max-w-sm text-center text-sm text-white/50">
          {mode === 'sign-up' ? 'Create your OMNIA account — one login works across Stays, Eats, and the agent.' : 'Welcome back to OMNIA Stays.'}
        </p>
        {mode === 'sign-up' ? (
