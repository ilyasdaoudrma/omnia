...
function AuthButtons() {
  if (!authEnabled) {
    return (
      <Link to="/trips">
        <Button variant="ghost" size="sm">Guest</Button>
      </Link>
    );
  }
  return (
    <>
      ...
      <SignedIn>
        <Link to="/trips"><Button variant="ghost" size="sm">My trips</Button></Link>
    ...
    >
      ...
      >
        <Link to="/" className="flex items-center gap-2 pl-1" data-cursor="hover">
          <Logo />
          <span className="hidden text-xs font-medium uppercase tracking-[0.25em] text-accent-soft sm:inline">Stays</span>
        </Link>
