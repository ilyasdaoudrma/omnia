...
const HERO_IMG = '/stays-hero.webp?v=2';

/**
 * Cinematic full-bleed hero for OMNIA Stays — a luxury riad at blue hour with a
 * slow Ken-Burns zoom and scroll-linked parallax on the copy, fading into the
 * light city picker below. A lighter cousin of the agent's eye hero.
 */
export function StaysHero() {
  ...
  return (
    <section ref={ref} className="relative h-[100svh] overflow-hidden">
      {/* Image (Ken-Burns zoom) as a CSS background with a warm-neutral fallback stacked behind. */}
      <motion.div
        style={{
          scale: imgScale,
          backgroundImage: `url('${HERO_IMG}'), radial-gradient(120% 100% at 50% 35%, #1c1712, #0a0806)`,
      ...
      {/* Just enough legibility scrim (kept light so the image stays bright) + fade into the page */}
      ...
      >
        ...
        >
          <Sparkles className="h-3.5 w-3.5" /> OMNIA Stays
        </motion.span>
        ...
        >
          Handpicked riads, lofts and villas across six Moroccan cities — booked in seconds,
