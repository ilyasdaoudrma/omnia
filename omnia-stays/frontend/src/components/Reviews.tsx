...
/** Guest reviews — public list + a star-rating form for signed-in users. */
export function Reviews({ listingId }: { listingId: string }) {
  const qc = useQueryClient();
  const { data: reviews = [], isLoading } = useQuery({ queryKey: ['reviews', listingId], queryFn: () => fetchReviews(listingId) });
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h3 className="font-serif text-2xl font-bold tracking-tight">Guest reviews</h3>
        {avg != null && (
      ...
      <GlassCard className="mt-4 p-5">
        {done ? (
          <p className="text-sm text-emerald-400">Thanks — your review was posted!</p>
        ) : needAuth ? (
          ...
          <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(); }} className="space-y-3">
            ...
            <textarea
              ...
              rows={3}
              maxLength={600}
              placeholder="Share a little about your stay…"
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full bg-[linear-gradient(135deg,#1d4ed8,#3b82f6)] px-5 py-2.5 text-sm font-semibold text-[#fff] transition-transform hover:scale-[1.02] disabled:opacity-60"
              data-cursor="hover"
            >
      ...
      <div className="mt-4 space-y-3">
        {isLoading ? (
          ...
          <p className="py-2 text-sm text-white/40">No guest reviews yet — be the first.</p>
        ) : (
          reviews.map((r) => (
