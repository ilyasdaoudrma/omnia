...
async function bootstrap() {
  ...
  // CORS — allow the configured frontend origins (Stays web + agent app).
  ...
  const origins = corsOrigins(config.get<string>('CORS_ORIGINS') ?? 'http://localhost:5181,http://localhost:5180');
  ...
  logger.log(`OMNIA Stays backend listening on http://localhost:${port}`);
}
