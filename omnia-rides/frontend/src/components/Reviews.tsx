...
/** Rider reviews — public list + a star-rating form for signed-in users. */
export function Reviews({ rideClassId }: { rideClassId: string }) {
  const qc = useQueryClient();
  const { data: reviews = [], isLoading } = useQuery({ queryKey: ['reviews', rideClassId], queryFn: () => fetchReviews(rideClassId) });
  ...
  const mutation = useMutation({
    mutationFn: () => createReview(rideClassId, { rating, comment: comment.trim() || undefined }),
    onSuccess: (res) => {
      if (res.needAuth) return setNeedAuth(true);
      if (res.ok) {
        setComment('');
        setDone(true);
        void qc.invalidateQueries({ queryKey: ['reviews', rideClassId] });
      }
    },
  });
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h3 className="font-serif text-2xl font-bold tracking-tight">Rider reviews</h3>
        {avg != null && (
      ...
      <GlassCard className="mt-4 p-5">
        {done ? (
          <p className="text-sm text-emerald-600">Thanks — your review was posted!</p>
        ) : needAuth ? (
          ...
          <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(); }} className="space-y-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full bg-[linear-gradient(135deg,#15803d,#22c55e)] px-5 py-2.5 text-sm font-semibold text-[#fff] transition-transform hover:scale-[1.02] disabled:opacity-60"
              data-cursor="hover"
            >
      ...
      <div className="mt-4 space-y-3">
        {isLoading ? (
          ...
          <p className="py-2 text-sm text-white/40">No rider reviews yet — be the first.</p>
        ) : (
          reviews.map((r) => (
